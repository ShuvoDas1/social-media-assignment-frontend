...
import { Post, PostRequest } from "@/features/post/postTypes";
import {
...
export default function FeedPage() {
  ...
  const removeImage = (index: number) => {
    setPostData({
      ...postData,
      images: postData.images.filter((_: any, i: number) => i !== index),
    });
  ...
  useEffect(() => {
    const notifyDropdown = document.querySelector<HTMLElement>("#_notify_drop");
    const notifyDropShowBtn =
      document.querySelector<HTMLElement>("#_notify_btn");
    ...
    notifyDropShowBtn?.addEventListener("click", handleClick);
    return () => notifyDropShowBtn?.removeEventListener("click", handleClick);
  }, []);
  ...
  const handlePostReact = async (postId: number, react: string) => {
    try {
      const res = await reactPost({ postId, react }).unwrap();
      toast.success("Post reacted successfully");
    } catch (error: any) {
      console.log(error);
                          ...
                          <div className="d-flex align-items-center gap-2 mt-2 mb-2">
                            ...
                            >
                              {postData.privacy === "public" ? (
                                <path
                                  fill="#666"
                                  d="M12 2a10 10 0 100 20 10 10 0 000-20zm0 18a8 8 0 01-6.32-3.09c.03-1.99 4.21-3.08 6.32-3.08s6.29 1.09 6.32 3.08A8 8 0 0112 20zm0-16a3.5 3.5 0 110 7 3.5 3.5 0 010-7z"
                                />
                              ) : (
                                <path
                                  fill="#666"
                                  d="M12 1a5 5 0 015 5v3h1a2 2 0 012 2v9a2 2 0 01-2 2H6a2 2 0 01-2-2v-9a2 2 0 012-2h1V6a5 5 0 015-5zm0 2a3 3 0 00-3 3v3h6V6a3 3 0 00-3-3z"
                                />
                              )}
                            </svg>
                            <select
                        ...
                        >
                          ...
                          <div className="_feed_inner_timeline_total_reacts _padd_r24 _padd_l24 _mar_b26">
                            ...
                            >
                              {post?.counts?.like > 0 &&
                                Array.from({
                                  length: Math.min(post.counts.like, 5),
                                }).map((_, index) => (
                                  <img
                                    key={index}
                                    src={`/assets/images/react_img${index + 1}.png`}
                                    alt="Image"
                                    className={`_react_img${index === 0 ? "1" : ""} ${
                            ...
                            <div className="_feed_inner_timeline_total_reacts_txt">
                              <p className="_feed_inner_timeline_total_reacts_para1">
                                <a href="#0">
                                  <span>{post?.counts?.comment}</span> Comment
                                </a>
                              </p>
